...
import { COOKIE_NAME, TEN_YEARS_IN_SECONDS, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, BUCKET, buildPublicUrl, createSupabaseClient } from '@/lib/config'
...
function validatePayload(payload: any) {
    ...
    if (!payload.birthYear || Number.isNaN(payload.birthYear)) missing.push('birth_year')
    if (!allowedSkinTypes.includes(payload.skinType)) missing.push('skin_type')
    if (!payload.profileImage) missing.push('profile_image')
    if (!Array.isArray(payload.skinConcernIds) || payload.skinConcernIds.length === 0)
        missing.push('skin_concern_ids')
    if (!Array.isArray(payload.onboardingAnswers) || payload.onboardingAnswers.length === 0)
        missing.push('onboarding_answers')

    if (missing.length) {
        return `Missing or invalid fields: ${missing.join(', ')}`
    }

    const invalidAnswers = payload.onboardingAnswers.some(
        (a: any) => typeof a.questionId !== 'number' || typeof a.answer !== 'string'
    )
    if (invalidAnswers) {
    ...
    if (payload.birthYear < 1798 || payload.birthYear >= currentYear) {
