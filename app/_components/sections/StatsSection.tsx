...
export function StatsSection({ itemsCount, expireCount, challengeCount }: StatsSectionProps) {
    const stats = [
        { id: "products", value: itemsCount, label: "Products", icon: "/icons/product.svg" },
        { id: "expiring", value: expireCount, label: "Expiring", icon: "/icons/expiring.svg" },
        { id: "challenge", value: challengeCount, label: "Challenge", icon: "/icons/challenge.svg" },
    ];

    return (
        <div className="grid xs:grid-cols-2 grid-cols-3 gap-4 w-full">
            {stats.map((item) => (
                <div key={item.id} className="bg-white p-4 rounded-xl w-full flex-start flex-col">
